...
```mermaid
flowchart TD
  subgraph group_runtime["Runtime and Channels"]
    node_main["Main Runtime<br/>[main.py]"]
    node_telegram_adapter["Telegram Adapter<br/>channel adapter"]
    node_whatsapp_adapter["WhatsApp Adapter<br/>channel adapter"]
    node_router["Shared Channel Router<br/>session router"]
  end
  subgraph group_conversation["Conversation and Policy"]
    node_flow["Conversation Flow<br/>orchestrator"]
    node_consent["Consent Gate<br/>consent stage"]
    node_loader["Scheme Loader<br/>loader and validator"]
    node_corpus["Scheme Rule Corpus<br/>scheme definitions"]
    node_profile["Worker Profile<br/>session model"]
    node_engine["Eligibility Engine<br/>deterministic evaluator"]
    node_operators["Rule Operators<br/>criteria operators"]
    node_content["Bilingual Content<br/>content catalog"]
  end
  subgraph group_output["Worker Output"]
    node_templates["Template Renderer<br/>localized renderer"]
    node_classifier["Occupation Classifier<br/>optional classifier"]
    node_checklist["Checklist Builder<br/>claim checklist"]
    node_pack["Application Pack<br/>claim pack composer"]
  end
  subgraph group_measurement["Privacy Measurement"]
    node_events[("Event Log<br/>SQLite event store")]
    node_report["Impact Reporter<br/>aggregate reporter"]
  end
  subgraph group_governance["Content Governance"]
    node_review_tool["Human Sign-off Tool<br/>terminal review tool"]
  end
  subgraph group_external["External Interfaces"]
    node_worker(("Worker<br/>external actor"))
    node_telegram_api{{"Telegram Bot API<br/>external interface"}}
    node_whatsapp_platform{{"WhatsApp/Meta Platform<br/>external interface"}}
    node_anthropic_api{{"Anthropic Messages API<br/>external API"}}
    node_reviewer(("Human Scheme Reviewer<br/>external actor"))
    node_official_sites{{"Official Scheme Websites<br/>external source"}}
  end
  node_worker -->|"starts screening"| node_main
  node_worker -->|"sends responses"| node_telegram_api
  node_telegram_adapter -->|"exchanges updates"| node_telegram_api
  node_worker -->|"sends responses"| node_whatsapp_platform
  node_whatsapp_platform -->|"delivers webhooks"| node_whatsapp_adapter
  node_main -->|"starts adapter"| node_telegram_adapter
  node_main -->|"starts adapter"| node_whatsapp_adapter
  node_main -->|"loads schemes"| node_loader
  node_loader -->|"reads definitions"| node_corpus
  node_main -->|"opens event log"| node_events
  node_telegram_adapter -->|"routes interactions"| node_router
  node_whatsapp_adapter -->|"routes interactions"| node_router
  node_router -->|"dispatches answers"| node_flow
  node_router -->|"reads localized text"| node_content
  node_flow -->|"gets consent"| node_consent
  node_flow -->|"records answers"| node_profile
  node_flow -->|"requests verdicts"| node_engine
  node_engine -->|"reads answers"| node_profile
  node_engine -->|"evaluates schemes"| node_corpus
  node_engine -->|"applies operators"| node_operators
  node_flow -->|"renders results"| node_templates
  node_flow -.->|"requests occupation"| node_classifier
  node_classifier -.->|"posts classification"| node_anthropic_api
  node_flow -->|"builds checklist"| node_checklist
  node_flow -->|"composes pack"| node_pack
  node_flow -->|"records screening"| node_events
  node_events -->|"supplies events"| node_report
  node_reviewer -->|"confirms values"| node_review_tool
  node_reviewer -->|"checks sources"| node_official_sites
  node_review_tool -->|"signs scheme fields"| node_corpus
  ...
  class node_main,node_telegram_adapter,node_whatsapp_adapter,node_router toneBlue
  class node_flow,node_consent,node_loader,node_corpus,node_profile,node_engine,node_operators,node_content toneAmber
  class node_templates,node_classifier,node_checklist,node_pack toneMint
  class node_events,node_report toneRose
  class node_review_tool toneIndigo
  class node_worker,node_telegram_api,node_whatsapp_platform,node_anthropic_api,node_reviewer,node_official_sites toneTeal
```
